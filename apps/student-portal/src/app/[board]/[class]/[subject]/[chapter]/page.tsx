...
export default async function ChapterPage({ params }: ChapterPageProps) {
  ...
  const isSeniorClass = resolvedParams.class === 'class-11' || resolvedParams.class === 'class-12';

  // For senior wing, try to load the standalone lesson file first
  if (isSeniorClass) {
    const lesson = await getSeniorLesson(resolvedParams.board, resolvedParams.class, resolvedParams.subject, chapterParam);
    if (lesson) {
